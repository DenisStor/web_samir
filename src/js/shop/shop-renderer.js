...
var ShopRenderer = (function() {
    ...
    function renderGallery(product, mainImage) {
        if (product.images && product.images.length > 1) {
            ...
            for (var i = 0; i < sortedImages.length; i++) {
                ...
                thumbsHtml += '<button class="gallery-thumb' + isActive + '" onclick="ShopApp.setGalleryImage(\'' + escapeAttr(img.url) + '\', this)">' +
            ...
            return '<div class="product-single-image gallery-main" title="Нажмите для увеличения">' +
                (imgSrc
    ...
    function getRelatedProducts(product, limit) {
        ...
        for (var i = 0; i < products.length; i++) {
            var p = products[i];
            if (p.id !== product.id && p.categoryId === product.categoryId && p.status === 'active') {
                related.push(p);
            }
        }
