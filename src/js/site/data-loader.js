            ...
            if (data.masters !== undefined) {
                ...
                if (mastersGrid) {
                    ...
                    if (activeMasters.length > 0) {
                        mastersGrid.innerHTML = activeMasters.map(SiteTemplates.createMasterCard).join('');
                    } else {
                ...
                (data.categories || []).forEach(function(category) {
                    ...
                    if (container) {
                        ...
                        if (services.length > 0) {
                            container.innerHTML = services.map(SiteTemplates.createServiceItem).join('');
                        } else {
            ...
            if (data.articles !== undefined) {
                ...
                if (blogGrid) {
                    ...
                    if (window.dynamicArticlesData.length > 0) {
                        ...
                        blogGrid.innerHTML = articlesToShow.map(SiteTemplates.createBlogCard).join('');
    ...
    async function loadSocial() {
        try {
            ...
            if (data.address) {
                ...
                if (siteAddress) siteAddress.innerHTML = escapeHTML(data.address).replace(/\n/g, '<br>');
            }
        } catch (error) {
            if (typeof console !== 'undefined' && console.warn) {
                console.warn('[DataLoader] Ошибка загрузки данных:', error.message || error);
